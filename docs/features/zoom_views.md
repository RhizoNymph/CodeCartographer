...
double-click an aggregated layout edge (count > 1) on the canvas
  ...
  -> pushFocusFrame({type:"edge", source, target})   (stack push, like any frame)
  -> fetchFramePayload: getEdgeDetail(source, target,
                   effectiveEdgeKinds(enabledEdgeKinds, viewMode))  (Tauri IPC)
       ^ the EFFECTIVE kinds of the view the fetch starts from, so a drill-in
         from module view expands exactly the Import aggregate the user
         double-clicked; a re-fetch from symbol view (popping back to an edge
         frame) honours the toolbar's enabled kinds
  ...
  -> store: viewMode="symbol", focusStack=[..., edge frame], focusEdgeDetail,
     focusNeighborhood=null, selectedNode cleared, bump layoutVersion
```

Edge frames ride the same stack as node frames: their breadcrumb chip reads
"source → target", they carry no depth/direction (the selectors hide), and the
canvas picks the payload via `focusLayoutIds(topFrame, neighborhood, edgeDetail)`.
...
- `crates/cc-core/src/model/graph.rs` — also: `EdgeDetail` +
  `CodeGraph::edge_detail` (subtree-to-subtree re-expansion of one aggregate,
  direction-scoped via `is_self_or_descendant`).
- `crates/cc-tauri/src/commands/parse.rs` — also: `get_edge_detail` command.
- `packages/app/src/canvas/renderers/PixiRenderer.ts` — edge double-tap
  hit-testing (`hitTestEdge` + `DOUBLE_TAP_MS`) dispatching `enterEdgeFocus`.
...
- **Edge focus is exact and directional.** `edge_detail` re-expands ONE
  aggregated source→target edge: only edges from the source subtree INTO the
  target subtree, never the reverse direction, never edges among siblings.
- **Edge frames have no depth or direction.** They show the members of one
  aggregate, not a BFS radius; the breadcrumb hides both selectors on them.
- **Exactly one focus payload is non-null while focused** — `focusNeighborhood`
  for a node top frame, `focusEdgeDetail` for an edge top frame — and both are
