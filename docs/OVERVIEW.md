Overview:
    ...
    data_flow:
        ...
        9. Focus / drill-down, described by a FocusFrame (node or edge) held in the store next to its fetched payload:
           - Node focus: the user focuses a node (F hotkey on the hovered/selected node, the canvas selection chip or Sidebar row Focus button, or a module-view File double-click) -> get_neighborhood(node_id, depth, edge_kinds) runs a bidirectional BFS in cc-core and returns the neighborhood node ids (incl. container chain) + direct edges.
           - Edge focus: the user double-clicks an AGGREGATED edge (count > 1) -> get_edge_detail(source_id, target_id, edge_kinds) runs cc-core's edge_detail, re-expanding that one aggregate into the underlying edges running from the source subtree into the target subtree (that direction only), plus their endpoints and container chain.
           Either way the store switches to symbol view and the canvas lays out ONLY the returned ids. A breadcrumb chip (node name + 1/2-hop depth selector, or "source -> target" with no depth selector, plus X) and Esc exit focus.
...
Features Index:
    ...
    zoom-views:
        description: Two zoom-level views selected by a toolbar segmented control. Module view (default on load) is the trustworthy zoomed-out import graph -- files render collapsed and only Import edges show, as DERIVED constraints over the user's saved state (never mutating it). Symbol view is the detailed expandable view with all enabled edge kinds. Focus mode is described by a FocusFrame and comes in two kinds, both of which lay out ONLY the fetched ids so the full symbol graph is never laid out: node focus drills into a bounded neighborhood (cc-core Neighborhood BFS via get_neighborhood), entered via the F hotkey (hovered node, falling back to the selected one), the canvas selection chip or Sidebar row Focus button, or a module-view File double-click; edge focus drills into ONE aggregated edge (cc-core edge_detail via get_edge_detail) and shows exactly the symbol pairs behind it, entered by double-clicking an aggregated edge (count > 1) on the canvas. Exit via the breadcrumb chip (node name + 1/2-hop depth + X, or "source -> target" + X -- depth does not apply to edge focus) or Esc. Focus actions are never placed in the hover tooltip, which unmounts on pointerout before it can be clicked. viewMode persists per folder.
        entry_points: [packages/app/src/stores/graphViewModel.ts, packages/app/src/stores/graphStore.ts, packages/app/src/canvas/Canvas.tsx, packages/app/src/canvas/FocusBreadcrumb.tsx, packages/app/src/canvas/SelectionChip.tsx, packages/app/src/canvas/focusHotkey.ts, packages/app/src/canvas/renderers/PixiRenderer.ts, crates/cc-core/src/model/graph.rs, crates/cc-tauri/src/commands/parse.rs]
        depends_on: [state-management, graph-layout, graph-model, resolution_precision]
        doc: docs/features/zoom_views.md

    state-management:
        description: Zustand stores for graph state (incl. viewMode and the focus frame + its fetched neighborhood/edge-detail payload), viewport state, per-view edge-kind counts published by the layout pass (edgeLegendStore), debug logging, and per-folder persistence (expanded/visible/viewMode). Pure view derivation + focus reducers live in graphViewModel.ts.
