...
impl LanguageSupport for TypeScriptSupport {
    ...
    fn collect_node_references(
        ...
        collect_ts_js_node_references(source, node, from_id, refs);
    }
...
/// Shared per-node reference collection logic for TypeScript and JavaScript.
fn collect_ts_js_node_references(
    source: &str,
    node: &Node,
    from_id: &NodeId,
    refs: &mut Vec<RawReference>,
