...
function friendlyEdgeLabel(kind: EdgeKind): string {
  switch (kind) {
    case "Import": return "Import";
    case "FunctionCall": return "Calls";
    case "MethodCall": return "Methods";
    case "TypeReference": return "Types";
    case "Inheritance": return "Extends";
    case "TraitImpl": return "Impls";
    case "VariableUsage": return "Vars";
  }
}

export const ViewSettingsPanel = memo(function ViewSettingsPanel({
  settings,
  onSettingsChange,
}: Props) {
  return (
    ...
    >
      ...
      {/* Far zoom opacity slider */}
      <div style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 10, color: "#64748b", marginBottom: 4 }}>
          Far zoom opacity: {Math.round(settings.minimapOpacity * 100)}%
        </div>
        <input
          type="range"
          min="0"
          ...
          onChange={(e) =>
            onSettingsChange({
              minimapOpacity: parseInt(e.target.value) / 100,
            })
          }
      ...
      {/* Medium zoom opacity slider */}
      <div style={{ marginBottom: 10 }}>
        <div style={{ fontSize: 10, color: "#64748b", marginBottom: 4 }}>
          Medium zoom opacity: {Math.round(settings.overviewOpacity * 100)}%
        </div>
        <input
          type="range"
          min="0"
          ...
          onChange={(e) =>
            onSettingsChange({
              overviewOpacity: parseInt(e.target.value) / 100,
            })
          }
          style={{ width: "100%", accentColor: "#3b82f6" }}
        />
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          {ALL_EDGE_KINDS.map((kind) => (
            <button
              ...
              title={`${settings.hideAtOverview.has(kind) ? "Show" : "Hide"} ${kind} at medium zoom`}
              style={{
                padding: "2px 5px",
                fontSize: 9,
